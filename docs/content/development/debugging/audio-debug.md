...
| 字段 | 说明 |
|------|------|
| `cf` | 波峰因子 dB（峰值与 RMS 之比，越高动态范围越大） |
| `ev` | 帧间能量方差（低值表示交叉淡入淡出导致的模糊） |
| `sc` | 频谱质心 Hz（频谱质量中心，语音约 1500–3000 Hz） |
| `sf` | 频谱平坦度（0–1，越高越接近噪声） |
| `sr` | 频谱滚降点 Hz（85% 能量阈值） |
| `zcr` | 过零率（每次采样的过零次数，语音约 0.05–0.15） |
| `dc` | 直流偏移（应接近 0.0，非零表示偏差） |
| `snr` | 估算信噪比 dB（越高越干净） |
| `lead` | 前导静音毫秒 |
| `trail` | 尾部静音毫秒 |
