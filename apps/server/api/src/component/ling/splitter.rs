...
fn emotion_regex() -> &'static Regex {
    ...
    RE.get_or_init(|| Regex::new(r"\[emotion:([a-z_]+)\]").unwrap())
}

fn bare_emotion_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"\[(happy|sad|angry|surprised|shocked|confused|neutral|loving|shy|smug|thinking|embarrassed|sad_smile|sad_tear|mouth_open|helpless|tears|speaking|listening|waiting)\]").unwrap())
}

fn parse_emotion_tag(text: &str) -> Option<String> {
    if let Some(caps) = emotion_regex().captures(text).ok().flatten() {
        return caps.get(1).map(|m| m.as_str().to_string());
    }
    if let Some(caps) = bare_emotion_regex().captures(text).ok().flatten() {
        return caps.get(1).map(|m| m.as_str().to_string());
    }
    None
}
...
fn strip_emotion_tags(text: &str) -> String {
    let text = emotion_regex().replace_all(text, "");
    bare_emotion_regex().replace_all(&text, "").to_string()
}

fn analyze_emotion_fallback(_text: &str) -> Option<String> {
...
impl Splitter {
    ...
    pub fn accept_token(&mut self, token: &str) -> Vec<Sentence> {
        ...
        if !self.first_chunk_emitted {
            ...
            if is_word_boundary(token) && !self.token_buffer.trim().is_empty() {
                ...
                let emotion =
                    parse_emotion_tag(&chunk).or_else(|| analyze_emotion_fallback(&chunk));
                result.push(Sentence {
                    text: strip_emotion_tags(&chunk),
                    emotion,
                });
            }
            return result;
        ...
        if sentences.is_empty() {
                ...
                && !self.text_collector.trim().is_empty()
            {
                ...
                self.last_flush_time = Instant::now();
                let emotion =
                    parse_emotion_tag(&remaining).or_else(|| analyze_emotion_fallback(&remaining));
                result.push(Sentence {
                    text: strip_emotion_tags(&remaining),
                    emotion,
                });
            }
            return result;
    ...
    pub fn accept_final(&mut self) -> Vec<Sentence> {
        let mut result = Vec::new();

        if !self.token_buffer.is_empty() {
            let chunk = self.token_buffer.clone();
            self.token_buffer.clear();
            let emotion = parse_emotion_tag(&chunk).or_else(|| analyze_emotion_fallback(&chunk));
            result.push(Sentence {
                text: strip_emotion_tags(&chunk),
                emotion,
            });
...
mod tests {
    ...
    fn test_splitter_abbreviation_not_split() {
        ...
        let all: Vec<Sentence> = r1.into_iter().chain(r2).collect();
        let joined: String = all.iter().map(|s| s.text.as_str()).collect();
        assert!(
    ...
    fn test_splitter_streaming_accumulation() {
        ...
        let r_final = splitter.accept_final();
        let all: Vec<Sentence> = r5.into_iter().chain(r_final).collect();
    ...
    fn test_splitter_token_first_chunk() {
        ...
        assert!(!r3[0].text.is_empty(), "chunk should not be empty");
    }
    ...
    fn test_splitter_token_max_chars() {
        ...
        for token in &tokens {
            let r = splitter.accept_token(token);
            if !r.is_empty() {
                assert!(!r[0].text.is_empty(), "chunk should not be empty");
                chunk_emitted = true;
                break;
            }
        }
    ...
    fn test_splitter_two_phase_integration() {
        ...
        let r_final = splitter.accept_final();
        let all: Vec<Sentence> = r3.into_iter().chain(r10).chain(r_final).collect();
        assert!(
            !all.is_empty(),
    ...
    fn test_splitter_no_duplication() {
        ...
        for token in tokens {
            ...
            for sentence in &r {
                assert!(
                    !seen.contains(&sentence.text),
                    "duplication detected: chunk '{}' already in '{}'",
                    sentence.text,
                    seen
                );
                seen.push_str(&sentence.text);
            }
            all.extend(r);
        }
        let r_final = splitter.accept_final();
    ...
    #[test]
    #[traced_test]
    fn test_parse_emotion_tag() {
        assert_eq!(
            super::parse_emotion_tag("[emotion:happy]"),
            Some("happy".to_string())
        );
        assert_eq!(
            super::parse_emotion_tag("[emotion:sad]"),
            Some("sad".to_string())
        );
        assert_eq!(
            super::parse_emotion_tag("[happy]"),
            Some("happy".to_string())
        );
        assert_eq!(super::parse_emotion_tag("hello"), None);
        assert_eq!(super::parse_emotion_tag("[notag]"), None);
    }

    #[test]
    #[traced_test]
    fn test_strip_emotion_tags() {
        assert_eq!(super::strip_emotion_tags("[emotion:happy]你好"), "你好");
        assert_eq!(super::strip_emotion_tags("你好[emotion:happy]"), "你好");
        assert_eq!(super::strip_emotion_tags("[happy]你好"), "你好");
        assert_eq!(super::strip_emotion_tags("你好"), "你好");
    }

    #[test]
    #[traced_test]
    fn test_analyze_emotion_fallback() {
        assert_eq!(
            super::analyze_emotion_fallback("今天天气很好"),
            Some("neutral".to_string())
        );
        assert_eq!(
            super::analyze_emotion_fallback("今天天气不好"),
            Some("neutral".to_string())
        );
        assert_eq!(
            super::analyze_emotion_fallback("桌子在房间里"),
            Some("neutral".to_string())
        );
    }
    ...
    #[test]
    #[traced_test]
    fn test_sentence_with_emotion() {
        let mut splitter = Splitter::new();
        let r1 = splitter.accept_token("[emotion:happy]你好！");
        let r_final = splitter.accept_final();
        let all: Vec<Sentence> = r1.into_iter().chain(r_final).collect();
        assert!(!all.is_empty(), "should produce output");
        assert_eq!(all[0].emotion, Some("happy".to_string()));
